...
| セクション | 内容 | レンダリング |
|-----------|------|------------|
| Hero | タグライン「いつものキーボードで／ローマ字の 1.7 倍の効率を」（「30 キー」は界隈語のため初心者向けに「いつものキーボード」へ翻訳。2026-09-25）（根拠: 統一ベンチマーク推定時間 QWERTY 3.195 → 新月 1.887 = 1.69 倍。直下に「※推定値」とベンチ記事への導線を置く）+ メインビジュアル (SVG)。「最高効率」等の最上級は統一ベンチマークで支持されないため使わない（2026-09-25） | prerender |
...
### 2.4 新月タイピング (`/practice`)

独立ページで新月配列の習得・復習・高速化・定点測定・IME実践を提供する。196件の語・短文と測定用8文を使用し、文字別の成績から出題を調整する。PC・タップ・IMEは分離して端末内に保存。研究を参考にした設計だが、医学的な最適性・最速の学習効果は主張しない。実装・計測・根拠と限界は `TRAINING.md` を参照。
