...
独立ページで新月配列の習得・復習・高速化・定点測定・IME実践を提供する。228件の語・短文と測定用8文を使用し、文字別の成績から出題を調整する。PC・タップ・IMEは分離して端末内に保存。研究を参考にした設計だが、医学的な最適性・最速の学習効果は主張しない。実装・計測・根拠と限界は `TRAINING.md` を参照。
