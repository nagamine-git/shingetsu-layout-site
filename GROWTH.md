...
### 実験 3: サイト内の新月専用タイピング練習（2026-09-20）

- ユーザー指定のローカル Keybr フォークを調査し、少数文字・苦手重点復習を参考にした独立実装を LP に追加。機能・制約・参照元は `PRACTICE.md`。
- 仮説: 導入前に打鍵順を覚えられ、自分の成績と復習候補が分かると、初回の体験価値と再訪理由を作れる。
- 主要指標は同期間・同入力方法の練習開始セッションに対する20文字完了セッションの割合。`practice_start` / `practice_complete` を追加。GA集計は未接続のため、転換率・継続率・学習効果は未判定。
- 公開境界は本機能を含む main デプロイの成功時刻で管理する。黒月デザイン・文字組み・CTA の変更と期間が重なるため、Star 増減をこの機能単独の効果と見なさない。
- 日次: 開始・完了イベントの取得可否と不具合。週次: PCとタップ別の途中離脱、導入遷移。月次: 短文コースや段階解放へ進むかを判断。データ未取得を0件と報告しない。
