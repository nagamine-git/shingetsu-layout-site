...
| ぶな v2.0 | 1.25 | 1.25 | 33 | 一部の濁音を単打に。3 打のかなが無い（v2.0 の定義。現行 v3.0rc3 は 30 キー） |
| 月配列 K | 1.26 | 1.26 | 40 | 数字段にかなを追加 |
| 月配列 5-532 | 1.27 | 1.25 | 32 | |
| 月配列 5-315 | 1.27 | 1.26 | 32 | ぶな配列の前身とされる（二次情報。同一系統を 2 行数えている可能性あり） |
...
| **新月（現行）** | **1.34** | **1.34** | **30** | 30 キー分の単打面は 2-263 と同一。「」は IME 既定の `[` `]` |
...
- 使用キーは **q〜p / a〜; / z〜/ の 30 キーのみ**。物理 `-` `[` `]` は一切使わない（現行の新月は ー「」に IME 既定の `-` `[` `]` も使える）
...
| 項目 | 内容 |
|---|---|
| 評価器 | 統一評価器 `evaluate.py`（Python）／最適化器は Rust 実装（`shingetsu_rs`）。時間モデルは両者で同一。**2026-09-25 時点でいずれも未公開** |
