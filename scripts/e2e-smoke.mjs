...
try {
  ...
  const snapshot = () => loop.evaluate(() => ({
    ...
    rate: document.getElementById("lab-rate").hidden ? "" : document.getElementById("lab-rate").textContent,
  ...
  check(/指の速さ [\d.]+打\/秒/.test(view.rate), `指の速さ: 打/秒を表示 (${view.rate.slice(0, 40)})`);
